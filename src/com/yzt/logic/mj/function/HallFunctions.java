...
public class HallFunctions extends TCPGameFunctions {
	...
	public synchronized static void interface_100007(WSClient channel, Map<String, Object> readData) {
		...
		Integer roomType = StringUtils.parseInt(readData.get("roomType"));// 房间类型 是否代开
		Integer zhangMaoType = StringUtils.parseInt(readData.get("zhangMaoType"));// 算不算分0,或者1.
		Integer woMaoType = StringUtils.parseInt(readData.get("woMaoType"));// 能不能握毛0,或者1.
		...
		room.setZhangMaoType(zhangMaoType);//长毛算不算钱. 非0就算钱
		room.setWoMaoType(woMaoType); //是否允许握毛,非0就是允许
		room.setLiangMaoChangJing(0); //亮毛场景0 
		// 初始化大接口的id
		...
		while (true) {
			room.setRoomId(CommonUtil.getGivenRamdonNum(6));// 设置随机房间号
		...
		if (roomType == null) {
			...
			// 设置用户信息
			p.setPosition(getWind(null));// 设置庄家位置,位置随机产生,如果是东风位置,直接设置为庄
		...
		readData.put("zhangMaoType", room.getZhangMaoType());
		readData.put("woMaoType", room.getWoMaoType());
	...
	public synchronized static void interface_100008(WSClient channel, Map<String, Object> readData) {
		...
		info.put("xjst", room.getXjst());
		info.put("woMaoType", room.getWoMaoType());
		info.put("zhangMaoType", room.getZhangMaoType());
		...
		if (hgetAll != null && hgetAll.size() > 0) {
			...
			for (String roomId : keySet) {
				...
				if (room != null && room.getCreateId().equals(userId) && room.getState() != Cnst.ROOM_STATE_YJS && room.getRoomType() == Cnst.ROOM_TYPE_2) {
					...
					map.put("zhangMaoType", room.getZhangMaoType());
					map.put("woMaoType", room.getWoMaoType());
