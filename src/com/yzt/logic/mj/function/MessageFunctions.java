...
public class MessageFunctions extends TCPGameFunctions {
	...
	public static Map<String, Object> getCurrentUserInfo(Player player, RoomResp room) {
		...
		if (room != null && player.getPlayStatus() == Cnst.PLAYER_STATE_GAME) {
			...
			currentUserInfo.put("liangList", player.getLiangList());
				...
				if (player.getActionList() != null && player.getActionList().size() > 0) {
					...
					for (Action action : player.getActionList()) {
						if (action.getType() == Cnst.ACTION_TYPE_CHI) {
							...
							map.put("action", -2);
							//map.put("extra", action.getActionId());
							//判断暗杠是否点了亮 .liangList的非0值是已经亮过的杠
							Integer tempPai = 0;
							if(player.getLiangList()!= null){
								for (int i = 0; i < tempList.size(); i++) {
									if(!tempList.get(i).equals(0) && tempList.get(i).equals(action.getExtra()) ){
										map.put("extra", action.getActionId());
										tempPai = action.getExtra();
									}
								}
	...
	public static Map<String, Object> getRoomInfo(RoomResp room,Player p) {
		...
		//开发选项
		roomInfo.put("woMaoType", room.getWoMaoType());
		roomInfo.put("zhangMaoType", room.getZhangMaoType());
		...
		if (room.getState() == Cnst.ROOM_STATE_GAMIING) {
			...
			roomInfo.put("position", room.getPosition());//风向的指示.
			...
			roomInfo.put("lastChuUserId", room.getLastChuPaiUserId());
	...
	public static void interface_100102(WSClient channel, Map<String, Object> readData) {
		...
		info.put("lastNum", room.getLastNum());//"剩余局（圈）数，为0的时候，要请求大结算"
		...
		for (Player p : players) {
			...
			map.put("liangList", p.getLiangList());
			if (p.getIsHu()) {
				map.put("isWin", 1);
			...
			if (p.getActionList() != null && p.getActionList().size() > 0) {
				List<Object> actionList = new ArrayList<Object>();
				List<Object> liangMaoList = new ArrayList<Object>();
	...
	public static void interface_100104(RoomResp room, List<Player> players, Integer interfaceId, Action action) {
		for (Player p : players) {
			...
			info.put("liangList", p.getLiangList());
			...
			if(action.getType() == Cnst.ACTION_TYPE_ANGANG){
				info.put("action", -2);
				//if(p.getUserId().equals(action.getUserId())){ //暗杠只能自己看.
	 ...
	 *            动作实体
	 */
	public static void interface_100104(RoomResp room, List<Player> players, Integer interfaceId, Action action,Integer userId) {
		for (Player p : players) {
			Map<String, Object> info = new HashMap<String, Object>();
			info.put("state", room.getState());
			info.put("userId", action.getUserId());
			...
			for (Player p1 : players) {
				if(p1.getUserId().intValue() == userId){
					info.put("liangList", p1.getLiangList());
				}
			}
